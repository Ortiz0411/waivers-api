...
export const getWaivers = async (): Promise<waiver[]> => {
    
    return query<waiver>('SELECT * FROM waivers ORDER BY id DESC')
}


export const getWaiverById = async (id: number): Promise<waiver> => {

    const rows = await query<waiver>('SELECT * FROM waivers WHERE id = $1', [id])
    return rows[0]
}


export const addWaiver = async (data: waiver): Promise<waiver> => {
  
    const rows = await query<waiver>(
        ...
        sendEmail(waiver).catch((err) =>
            console.error('Erro en enviar email:', err)
        )
